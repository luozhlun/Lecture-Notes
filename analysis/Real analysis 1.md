...
Let $\mathcal{A}\subset P(X)$ be an algebra, $\mu_0$ a premeasure on $\mathcal{A}$, $\sigma(\mathcal{A})$ the $\sigma$-algebra generated by $\mathcal{A}$, and $\mu^\ast$ the outer measure defined as

$$
\mu^\ast(E)=\inf\left\{\sum_{j=1}^\infty\mu_0(A_j):A_j\in\mathcal{A},E\subset\bigcup_{j=1}^\infty A_j\right\}, \text{then}
$$

1. $\mu=\mu^\ast|_\sigma(\mathcal{A})$ is a measure on $\sigma(\mathcal{A})$ with $\mu|_\mathcal{A}=\mu_0$.
2. If $\nu$ is another measure on $\sigma(\mathcal{A})$ with $\nu|_\mathcal{A}=\mu_0$, then $\nu(E)\leq\mu(E)$ for all $E\in\sigma(\mathcal{A})$, with equality if $\mu(E)<\infty$.
3. If $\mu_0$ is $\sigma-$finite, then $\mu$ is the unique extension of $\mu_0$ to a measure on $\sigma(\mathcal{A})$.

$\bf Rmk$

$\mu$ may not be a complete measure on $\sigma(\mathcal{A})$. By Caratheodory's theorem, $\mu$ is a complete measure on $\{\mu^\ast\text{-measurable sets}\}$. But in general, $\sigma(\mathcal{A})\subset\{\mu^\ast\text{-measurable sets}\}$.
